...
const getCustomersByEmployeeData = async <T,>(ID: string): Promise<T> => {
  const res = await fetch(
    process.env.NEXT_PUBLIC_URL + "/api/customersByEmployee",
    {
      method: "POST",
      body: JSON.stringify({
        ID: ID,
...
const EmployeeTable: React.FC<EmployeeTableProps> = (
  ...
  const onMount = async (): Promise<void> => {
    ...
    const customers = await getCustomersByEmployeeData<Employee>(
      state.selectedID
    );
  ...
  return (
    <>
      {state.isToggled ? (
        <>
          <EmployeeDetail
            customers={employee.employee ? employee.employee.customers : []}
