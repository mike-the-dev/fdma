        ...
        <Chip
          ...
          startContent={
            <Icon
              className="text-sm"
              icon={
                bankAccount.status?.toLowerCase() === "active" ||
                bankAccount.status?.toLowerCase() === "verified"
                  ? "lucide:check-circle"
                  : bankAccount.status?.toLowerCase() === "new" ||
                      bankAccount.status?.toLowerCase() === "pending"
                    ? "lucide:clock"
                    : "lucide:alert-circle"
              }
            />
          ...
          variant="flat"
        >
          {(bankAccount.status || "unknown").charAt(0).toUpperCase() +
            (bankAccount.status || "unknown").slice(1)}
        </Chip>
      </CardHeader>
      <Divider />
      <CardBody>
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mb-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-md bg-primary-100 dark:bg-primary-900/30">
                  <Icon
                    className="text-xl text-primary-500 dark:text-primary-400"
                    icon="lucide:building-bank"
                  />
                </div>
                <div>
                  <p className="font-medium">
                    {(bankAccount as any).bank_name || "Unknown Bank"}
                  </p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    {(bankAccount as any).country?.toUpperCase()} •{" "}
                    {formatCurrency(bankAccount.currency || "usd")}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                ...
                <div>
                  ...
                  <p className="font-mono">
                    {(bankAccount as any).routing_number || "Not available"}
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">
                    {(bankAccount as any).account_holder_name ||
                      "Not specified"}
                  </span>
                </div>
                  ...
                  <span className="font-medium">
                    {(bankAccount as any).account_holder_type
                      ? (bankAccount as any).account_holder_type
                          .charAt(0)
                          .toUpperCase() +
                        (bankAccount as any).account_holder_type.slice(1)
                      : "Not specified"}
                  </span>
                </div>
                <Divider className="my-2" />
                <div className="flex items-center justify-between">
                  ...
                  <Chip
                    color={
                      bankAccount.default_for_currency ? "success" : "default"
                    }
                    size="sm"
                    variant="flat"
                  >
