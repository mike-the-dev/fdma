...
import {
  ...
  useAccounts,
} from "../customerInsights.service";
import {
  validateAccountId,
  validateDollars,
  validatePercent,
} from "../_shared/customerInsights.validators";
...
export const useCustomerInsightsForm = (): UseCustomerInsightsFormReturn => {
  ...
  const validators: CustomerInsightsValidators = {
    ...
    avgSpentDollars: {
      onChange: ({ value }: { value: string }) => validateDollars(value),
    },
    subscriptionRatePercent: {
      onChange: ({ value }: { value: string }) => validatePercent(value),
    },
    repeatRatePercent: {
      onChange: ({ value }: { value: string }) => validatePercent(value),
    },
