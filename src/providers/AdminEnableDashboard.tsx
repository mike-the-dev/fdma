...
const AdminEnableDashboard: React.FC<AdminEnableDashboardProps> = (
  props
): React.ReactElement => {
  const context = useGlobalState();

  React.useEffect(() => {
    const authAdminPublicPermission =
      localStorage.getItem("auth-admin-public-permission") || "";
    ...
    const keydownHandler = (event: KeyboardEvent) => {
      if (event.ctrlKey && event.shiftKey && event.key === "`") {
        ...
        context.setState((prevState) => ({
          isAdminDashboardEnabled: !prevState.isAdminDashboardEnabled,
        }));
      }
    };
