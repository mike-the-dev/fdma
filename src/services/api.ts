...
export const handleRequest = async <T>(request: Promise<any>): Promise<T> => {
  try {
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (!error.response) {
        throw new Error("Network error: Could not reach the server or CORS rejection.");
      }
...
export const getCookieData = (): { accountID: string; accountCompany: string } => {
  const profile = typeof window !== "undefined" ? localStorage.getItem("instapaytient_profile") : null;
  if (!profile) {
  ...
  if (!parsed.accountID || !parsed.accountCompany) {
    throw new Error("Account ID or account company is missing from the profile.");
  }
