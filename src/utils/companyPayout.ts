...
const companyPayout = async (payoutAccountID: string, currentSubtotalPrice: number, payoutMetaData: PayoutMetaData): Promise<void> => {
  const stripeIDs: { accountID: string, stripeID: string, take: number }[] = [
    // Michael
    { accountID: "e#01HE9A1R4140ZZRB2KNSCWTR4S", stripeID: "acct_1NlGo22VtceChzVP", take: 1 },
    // Henry
    { accountID: "e#01HE9A3S9QFQMJQC0328HWNFN7", stripeID: "acct_1NlGoNFbIMQZZWcr", take: 1 },
    // Sally
    { accountID: "e#01H96PE28NS6D76CVTJRKDYD9K", stripeID: "acct_1NlGnKFZCuSOUdyG", take: 0.35 }
  ];

  stripeIDs.forEach(async founder => await founderPayout(founder.accountID, founder.stripeID, founder.take, currentSubtotalPrice, payoutMetaData));
  ...
  employeeIdentifications.forEach(async employee => await employeePayout(employee.id, employee.stripeID, payoutAccountID, currentSubtotalPrice, payoutMetaData));
};
