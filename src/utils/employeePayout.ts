...
const employeePayout = async (
  ...
  payoutMetaData: any): Promise<void> => {
  ...
  const customer = response.Items?.filter(customer => customer.customerID === customerID) || [];

  if (customer.length === 0) return console.log(`Employee ${employeeID} is not assigned to this customer.`);
  ...
  const payoutAmountAsText: string = currencyFormatter.format(amountToPayout / 100, { code: 'USD' });
  ...
  console.log(`${employeeID} payoutAmountAsText from ${customer[0].name} sale of ${currencyFormatter.format(currentSubtotalPrice, { code: 'USD' }) }: `, payoutAmountAsText);
