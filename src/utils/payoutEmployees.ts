...
const payoutEmployees = async (
  payload: PayoutEmployeesInputParams
): Promise<any> => {
  const shopifyData = payload;

  if (!shopifyData.payout_account_id)
    throw new Error("Account ID for payout user was not given");
  ...
  if (!shopifyData.current_subtotal_price)
    throw new Error(
      `Current sub total price not available. Check Shopify flow app settings to make sure action as been set up properly. current_subtotal_price: ${shopifyData.current_subtotal_price} payout_account_id: ${shopifyData.payout_account_id}`
    );
