...
const verifyMagicLink = async (data: VerifyMagicLinkData): Promise<VerifyMagicLinkResponse> => {
  try {
    // Call NestJS backend directly - no authentication needed for magic link verification
    const response = await apiClient.post('/api/verifyMagicLinkSuperAdmin', data);
